...
public class BaseEnemy : MonoBehaviour, IPoolingObject
{
    ...
    public void DropItem()
    {
        for (int i = 0; i < m_eEnemyDropItems.dropItems.Count; i++)
        {
            ...
            {
                ThirdPersonMovement.Instance._Inventory.GetItem(m_eEnemyDropItems.dropItems[i].itemID);
                //Inventory.Instance.GetItem(0);

            }
        }
        ThirdPersonMovement.Instance._Inventory.AddGold(m_eEnemyDropItems.gold);
